...
class DataDesaExport implements FromArray, WithHeadings, WithCustomStartCell, WithEvents
{
    protected $kode_desa;
    protected $query;

    // Menambahkan konstruktor untuk menerima parameter filter
    public function __construct($kode_desa = null, $query = null)
    {
        $this->kode_desa = $kode_desa; // Jenis kelamin
        $this->query = $query; // Pencarian
    }

    public function array(): array
    {
        $query = DataDesaModel::select('kode_desa', 'nama_desa', 'kode_kecamatan', 'remark');
        // Filter berdasarkan jenis kelamin jika ada
        if ($this->kode_desa) {
            $query->where('kode_desa', $this->kode_desa);
        }

        // Filter berdasarkan pencarian query (misalnya berdasarkan nama atau NIK)
        if ($this->query) {
            $query->where(function ($subQuery) {
                $subQuery->where('kode_desa', 'like', '%' . $this->query . '%')
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->setCellValue('A2', 'Laporan Data Desa');
