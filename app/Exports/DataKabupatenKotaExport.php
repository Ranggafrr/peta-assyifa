...
class DataKabupatenKotaExport implements FromArray, WithHeadings, WithCustomStartCell, WithEvents
{
    protected $kode_kabupaten_kota;
    protected $query;

    // Menambahkan konstruktor untuk menerima parameter filter
    public function __construct($kode_kabupaten_kota = null, $query = null)
    {
        $this->kode_kabupaten_kota = $kode_kabupaten_kota; // Jenis kelamin
        $this->query = $query; // Pencarian
    }

    public function array(): array
    {
        $query = DataKabupatenKotaModel::select('kode_kabupaten_kota', 'nama_kabupaten_kota', 'kode_Propinsi', 'remark');
        // Filter berdasarkan jenis kelamin jika ada
        if ($this->kode_kabupaten_kota) {
            $query->where('kode_kabupaten_kota', $this->kode_kabupaten_kota);
        }

        // Filter berdasarkan pencarian query (misalnya berdasarkan nama atau NIK)
        if ($this->query) {
            $query->where(function ($subQuery) {
                $subQuery->where('kode_kabupaten_kota', 'like', '%' . $this->query . '%')
