...
@section('content')
    <div class="mx-5 mt-3 border bg-white rounded-lg py-5">
        <form action="{{ route('access-sub-menu.store') }}" method="post">
            @method('POST')
            @csrf
            <div class="flex justify-between mx-5 items-center">
                ...
                <div class="flex justify-end gap-x-2">
                    <a href="{{ route('access-sub-menu.index') }}"
            ...
            <div class="grid grid-cols-4 gap-4 m-5 pb-4">
                <div class="w-full">
                    <x-search-select name="role_id" label="Role" :options="$ListRole" placeholder="Pilih role..."
                        :hasSearch="true" :required="true" />
                </div>
                <div class="w-full">
                    <x-search-select name="kode_submenu" label="Sub menu" :options="$ListMenu" placeholder="Pilih sub menu..."
                        :hasSearch="true" :required="true" />
                </div>
                <div class="w-full">
