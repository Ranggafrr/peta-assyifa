...
@section('content')
    <div class="mx-5 mt-3 border bg-white rounded-lg py-5">
        <div class="flex justify-between mx-5 items-center">
            ...
            <a href="{{ route('access-sub-menu.create') }}" class="btn-primary inline-flex gap-x-2"> <i data-lucide="plus"
                                        ...
                                        @foreach ($data as $item)
                                            <tr>
                                                ...
                                                <td class="table-body-col">
                                                    {{ $item->nama_submenu ?? '-' }} / {{ $item->kode_submenu ?? '-' }}
                                                </td>
                                                <td class="table-body-col">
                                                    {{ $item->role ?? '-' }}
                                                </td>
                                                <td class="table-body-col">
                                                    <form id="form-{{ $item->id }}"
                                                        action="{{ route('access-sub-menu.update', ['access_sub_menu' => $item->id]) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('PUT')
