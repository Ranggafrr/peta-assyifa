...
<div class="mx-5 mt-3 border bg-white rounded-lg py-5">
    <div class="flex justify-between mx-5 items-center">
        ...
        <div class="flex gap-4">
            <a href="{{ route('data-pendidikan.create') }}" class="btn-primary inline-flex gap-x-2"> <i
                    data-lucide="plus" class="size-4"></i> Tambah
                Data</a>
            <a href="{{ route('data-pendidikan.saveToExcel') }}" class="btn-success inline-flex gap-x-2"> <i
    ...
    <form action="" method="get" class="flex items-center space-x-2 mx-5 mt-5">
        ...
        <div class="w-2/12">
            <select class="form-select" onchange="this.form.submit()" name="akreditasi">
                <option value="" disabled selected>Akreditasi</option>
                <option {{ $akreditasi == 'all' ? 'selected' : '' }} value="all">Semua</option>
                <option {{ $akreditasi == 'A' ? 'selected' : '' }} value="A">A</option>
                <option {{ $akreditasi == 'B' ? 'selected' : '' }} value="B">B</option>
                <option {{ $akreditasi == 'C' ? 'selected' : '' }} value="C">C</option>
                <option {{ $akreditasi == 'Tidak Terakreditasi' ? 'selected' : '' }} value="Tidak Terakreditasi">Tidak
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-100">
                                <tr>
                                    ...
                                    <th scope="col" class="table-head-row">Nama Pendidikan</th>
                                    <th scope="col" class="table-head-row">Tingkat Pendidikan</th>
                                    <th scope="col" class="table-head-row">Akreditasi</th>
                                ...
                                @if ($data->isNotEmpty())
                                    @foreach($data as $index => $item)
                                        <tr>
                                            ...
                                            <td class="table-body-col">{{ $item->nama_pendidikan }}</td>
                                            <td class="table-body-col">{{ $item->tingkat_pendidikan }}</td>
                                            <td class="table-body-col">{{ $item->akreditasi }}</td>
                                                    ...
                                                    <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden bg-white shadow-md rounded-lg mt-2 min-w-40 z-[60]"
                                                        ...
                                                        <div class="flex flex-col p-1 space-y-0.5">
                                                            <a class="inline-flex items-center gap-x-3.5 py-2 px-3 w-full rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100"
                                                                href="{{ route('data-pendidikan.edit', ['data_pendidikan' => $item->id]) }}">
                                                                <i data-lucide="square-pen" class="size-4"></i> Edit
                                                                Data
                                                            </a>
                                                            <a class="inline-flex items-center gap-x-3.5 py-2 px-3 w-full rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100"
                                                                href="{{ route('data-pendidikan.show', ['data_pendidikan' => $item->id]) }}">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                const actionUrl = `{{ route('data-pendidikan.destroy', ':data-pendidikan') }}`.replace(':data-pendidikan',
                    dataId);
