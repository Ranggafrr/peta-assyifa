...
<div class="mx-5 mt-3 border bg-white rounded-lg py-5">
    <form action="{{ route('master-golongan-darah.store') }}" method="post">
        @method('POST')
        @csrf
        ...
        <div class="grid grid-cols-4 gap-4 m-5 pb-4">
            <div class="w-full">
                <x-search-select name="golongan_darah" label="Golongan Darah" :options="['A' => 'A', 'B' => 'B', 'AB' => 'AB', 'O' => 'O', 'A+' => 'A+', 'A-' => 'A-', 'B+' => 'B+', 'B-' => 'B-', 'AB+' => 'AB+', 'AB-' => 'AB-', 'O+' => 'O+', 'O-' => 'O-']" placeholder="Pilih golongan darah..." :hasSearch="false"
                    :required="true" />
            </div>
            <div class="w-full">
