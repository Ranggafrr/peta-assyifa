...
<div class="mx-5 mt-3 border bg-white rounded-lg py-5">
    <div class="flex justify-between mx-5 items-center">
        ...
        <div class="flex gap-4">
            <a href="{{ route('data-pekerjaan.create') }}" class="btn-primary inline-flex gap-x-2"> <i
                    data-lucide="plus" class="size-4"></i> Tambah
                Data</a>
            <a href="{{ route('data-pekerjaan.saveToExcel') }}" class="btn-success inline-flex gap-x-2"> <i
    ...
    <form action="" method="get" class="flex items-center space-x-2 mx-5 mt-5">
        ...
        <div class="w-1/4">
            <select class="form-select" onchange="this.form.submit()" name="status_pekerjaan">
                <option value="" disabled selected>Pilih Status Pekerjaan</option>
                <option {{ $status_pekerjaan == 'all' ? 'selected' : '' }} value="all">Semua</option>
                <option {{ $status_pekerjaan == 'Penuh Waktu' ? 'selected' : '' }} value="Penuh Waktu">Penuh Waktu</option>
                <option {{ $status_pekerjaan == 'Paruh Waktu' ? 'selected' : '' }} value="Paruh Waktu">Paruh Waktu</option>
            </select>
        </div>
    </form>
    <div class="mx-5 mt-5">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-100">
                                <tr>
                                    ...
                                    <th scope="col" class="table-head-row">Nama Pekerjan</th>
                                    <th scope="col" class="table-head-row">Status Pekerjan</th>
                                ...
                                @if ($data->isNotEmpty())
                                    @foreach($data as $index => $item)
                                        <tr>
                                            ...
                                            <td class="table-body-col">{{ $item->nama_pekerjaan }}</td>
                                            <td class="table-body-col">{{ $item->status_pekerjaan }}</td>
                                                    ...
                                                    <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden bg-white shadow-md rounded-lg mt-2 min-w-40 z-[60]"
                                                        ...
                                                        <div class="flex flex-col p-1 space-y-0.5">
                                                            <a class="inline-flex items-center gap-x-3.5 py-2 px-3 w-full rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100"
                                                                href="{{ route('data-pekerjaan.edit', ['data_pekerjaan' => $item->id]) }}">
                                                                <i data-lucide="square-pen" class="size-4"></i> Edit
                                                                Data
                                                            </a>
                                                            <a class="inline-flex items-center gap-x-3.5 py-2 px-3 w-full rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100"
                                                                href="{{ route('data-pekerjaan.show', ['data_pekerjaan' => $item->id]) }}">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                const actionUrl = `{{ route('data-pekerjaan.destroy', ':data_pekerjaan') }}`.replace(':data_pekerjaan',
                    dataId);
