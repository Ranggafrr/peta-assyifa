...
@section('content')
    <div class="mx-5 mt-3 border bg-white rounded-lg py-5">
        <form action="{{ route('data-rt.update', ['data_rt' => $data->id]) }}" method="post">
            @method('PUT')
            @csrf
            <div class="flex justify-between mx-5 items-center">
                <p class="font-semibold text-zinc-800 text-lg">{{ $page }}</p>
                <div class="flex justify-end gap-x-2">
                    <a href="{{ route('data-rt.index') }}" class="btn-outline-primary inline-flex items-center gap-x-2">
            ...
            <div class="grid grid-cols-4 gap-4 m-5 pb-4">
                <div class="w-full">
                    <x-Input type="text" name="kode_rt" label="Kode Rt" :required="true" :value="$data->kode_rt" />
                </div>
                <div class="w-full">
                    <x-Input type="text" name="nama_rt" label="Nama Rt" :required="true" :value="$data->nama_rt" />
                </div>
                <div class="w-full">
                    <x-Input type="text" name="kode_rw" label="Kode Rw" :required="true" :value="$data->kode_rw" />
                </div>
                <div class="w-full">
